namespace DynamicSpecs.Core
{
    ...
    public abstract class WorkflowSpecification : ISpecify
    {
        ...
        {
            var typeAttributes = GetType().GetRuntimeFields().Select(x => x.GetCustomAttributes());
            var typeRequests = typeAttributes.Where(x => x.GetType() == typeof(RequestTypeAttribute))
